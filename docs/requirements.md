...
- **H1b 收益率口径（2026-08-02 用户确认修订）**：主口径 = 期初资产基准，基准 = 期初资金 + 期初持仓变现估值（≈ 卖出未配对金额）；收益率 = (期末资产 − 基准 − 净转入) / 基准。完整历史（期初资产=0）退化为「累计入金基准」：收益率 = (期末资产 − 累计入金) / 累计入金。出入金分别统计（累计入金/累计出金/净转入），出金经净转入体现，不会把出金误当亏损；高频出入金账户两口径差异显著。
...
2. 期初资金 / 期末资金（资金余额口径）
3. 净转入资金（银行转证券 − 证券转银行）
4. 总收益率（H1b 口径：期初资产基准，完整历史退化为累计入金基准）+ 净转入（累计入金/累计出金单独统计）
5. 年化收益率（按区间天数折算）
6. 累计已实现盈亏（FIFO，含费用）
