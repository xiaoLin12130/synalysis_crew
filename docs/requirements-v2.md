...
- 基于 TWR 收益率曲线（v2.1）`v = 1 + R`：
  ...
  - **腰斩次数（v2.2 修订，用户确认）**：递进式计数——基准腰线 floor 初始 = 1.0（初始净值）；
    依次遍历曲线：v > floor 时 floor = v（新高重置）；v ≤ floor × 0.5 时计 1 次并把
    floor 更新为 v（即 1 → 0.5 第 1 次，0.5 → 0.25 第 2 次，0.25 → 0.125 第 3 次，
    连续下跌也逐级计数）；回升超过当前 floor 才重置基线。
- 字段 `pnl.double_count / pnl.halved_count`，前端文案「账户翻倍次数 / 账户腰斩次数」。

### 1.8 历史记录删除与运行中任务（v2.2）
- 历史分析记录支持删除：`DELETE /api/analyses/{id}`，删除成功 204；不存在 404 中文；
  前端侧栏每项提供删除按钮（二次确认），删除后列表刷新。
- 分析任务进行中点击历史记录：**不得丢失进行中的任务**——任务继续在后台运行并轮询，
  侧栏显示「分析进行中 n%」指示与「回到分析」按钮；任务完成后可从历史或指示器回到结果页。

### 1.9 交易明细排序与盈亏比例（v2.2）
- 交易明细表格列：股票(代码/名称)、买入(量/金额)、卖出(量/金额)、**盈亏比例(pnl÷买入金额)**、
  盈亏、持股天数、起止日期、状态；
...
DELETE /api/analyses/{id} → 204 / 404（中文）
```
